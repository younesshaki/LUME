 ...
 * types so a correct-sounding answer cannot hide a UI no-op. A scenario can
 * also store a verified result count and require a later result to equal it;
 * this avoids baking volatile production-backed inventory counts into tests.
 *
 * Usage:
...
function resultCountFromContent(content) {
  const match =
    /\b([\d,]+)\s+matching\s+vehicles?\s+found\b/i.exec(content) ??
    /\b([\d,]+)\s+matching\b/i.exec(content);
...
async function runScenario(scenario) {
  ...
  let sessionId;
  let failures = 0;
  const storedResultCounts = new Map();
  ...
  for (const step of scenario.turns) {
    ...
    } = await sendTurn(messages, sessionId);
    sessionId = nextSessionId;
    ...
    if (typeof step === "object") {
      const resultCount = resultCountFromContent(content);
      if (step.storeResultCountAs) {
        if (resultCount === null) {
          failures += 1;
          console.log(
            `  ✗ FAIL: could not store a result count as "${step.storeResultCountAs}"`,
          );
        } else {
          storedResultCounts.set(step.storeResultCountAs, resultCount);
        }
      }
      if (step.expectStoredResultCount) {
        const expectedCount = storedResultCounts.get(
          step.expectStoredResultCount,
        );
        if (expectedCount === undefined) {
          failures += 1;
          console.log(
            `  ✗ FAIL: no stored result count named "${step.expectStoredResultCount}"`,
          );
        } else if (resultCount !== expectedCount) {
          failures += 1;
          console.log(
            `  ✗ FAIL: expected ${expectedCount} matching vehicles, received ${resultCount ?? "no count"}`,
          );
        }
      }
      if (
        step.expect &&
        !content.toLowerCase().includes(step.expect.toLowerCase())
      ) {
        failures += 1;
      ...
      if (
        step.reject &&
        content.toLowerCase().includes(step.reject.toLowerCase())
      ) {
        failures += 1;
      ...
      if (step.rejectAction && actionTypes.includes(step.rejectAction)) {
        failures += 1;
        console.log(
          `  ✗ FAIL: action type "${step.rejectAction}" was unexpectedly emitted`,
        );
      }
      if (step.expectActionFields) {
        ...
        if (!matchingAction) {
          failures += 1;
          console.log(
            `  ✗ FAIL: no action matched fields ${JSON.stringify(step.expectActionFields)}`,
          );
        }
      }
      if (step.rejectActionFields) {
        ...
        if (rejectedAction) {
          failures += 1;
          console.log(
            `  ✗ FAIL: an action unexpectedly matched fields ${JSON.stringify(step.rejectActionFields)}`,
          );
        }
      }
    }
...
async function main() {
  ...
  const scenarioIndex =
    scenarioIndexValue === undefined ? null : Number(scenarioIndexValue);
  const scenarioPattern = process.env.CONCIERGE_TEST_SCENARIO_PATTERN;
  const selectedScenarios =
    scenarioIndex !== null
      ? Number.isSafeInteger(scenarioIndex) &&
        scenarioIndex >= 0 &&
  ...
  for (const result of results) {
    ...
    console.log(
      `${result.failures === 0 ? "✓" : "✗"} ${result.name}${result.failures ? ` (${result.failures} failing check(s))` : ""}`,
    );
  }
  console.log(
