...
- Numeral ordinals, whole/entire resets, make switches, selection resets, and
  ...
  sequence tests.
- There's also a **softer, different problem**: when the model *does* have to
...
This layer also recognizes a small set of specific *moves*: "open the first
...
The concierge keeps a small memory *per visitor session*: which filters are
...
Every turn, this layer decides: *do we need to run a new database search, or
does the visitor's message just reference something already on screen?*
...
- "Do you have a 2026 Camry?" → runs the real query, states the real count and
  examples.
- A refinement that matches nothing → says so plainly, names *why* (e.g. "no
...
could pinpoint the *exact* line of logic responsible, not guess.
...
and matching a *make* to a *model* (or vice versa) now requires the first
...
mis-matched onto the *model* filter (only the *make* matching had the
...
**What happened:** fixing the word-matching wasn't enough — the *old* model
("Camry") was still sitting in memory from the previous message and got
combined with the *new* make ("Cadillac"). **Fix:** naming a different make
...
**What happened:** fix #5 cleared the old *model* on a make-switch, but not
the old *year*. So "year: 2026" — set three messages earlier — silently
...
changes, on the reasoning that a specific model-year is tied to *the car you
were just discussing*, not a standing preference like price range is.

### 7. Once *any* refinement returned zero results, the concierge got permanently stuck
...
matches" only protected the *old list of cars shown* — it never rolled back
the *filters themselves*. So every failed attempt left a permanent scar on the
...
never the stored *selection* (or the result list it came from); the stale
...
*sequence* of turns — selection, filter change, reset — which is why the
...
also clears the old body class. Price, location, mileage, drivetrain, and
other standing preferences remain intact.
...
accidentally tripped a *different*, weaker rule (meant for things like "not
...
— which answered with things like *"priced 10% below comparable listings"* and
*"near-current model year"*. Nothing in our real data supports those specific
...
about recognizing the *request*, it's about the AI's free-form *answer* not
...
visitor's screen — asking it to resolve this itself is *strictly worse* than
...
- For the specific, narrow question of *"what is the visitor's intent"* (reset?
...
*answer*, not its recognition of the *request*. That's a different mechanism
