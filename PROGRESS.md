...
## SCRUM-182 Drag-and-drop page block editor
- Status: done
- Files: packages/blocks/src/{editorOrdering,editorOrdering.test,index}.ts, apps/admin/app/admin/[tenant]/pages/[pageId]/PageEditorClient.tsx
- Migration: none
- Env added: none
- Review notes / open questions: The existing editor already provided the requested three-column layout, Zod-backed block descriptors, generated property controls, draft persistence, `pages.updated_at` touching, validation, live preview, publish/unpublish, and revision restore. This ticket fills the actual gap: palette items are grouped by content/data/media category and existing blocks can be reordered by a dedicated native drag handle with a visible drop target. The pure immutable reorder operation is unit-tested. Up/Down buttons remain beside every drag handle as the keyboard-accessible equivalent, avoiding a pointer-only editor. The current controlled field renderer consumes editor descriptors derived from the Zod registry and validates against the real schemas on save; it was deliberately retained instead of adding react-hook-form and rewriting stable array/media controls solely to match the Jira implementation suggestion. Claude should confirm that functional schema-driven equivalence is acceptable.

## SCRUM-185 Tenant asset picker
- Status: done
- Files: apps/admin/components/asset-picker.tsx, apps/admin/lib/assets*, apps/admin/app/admin/[tenant]/{assets/AssetsClient,pages/[pageId]/PageEditorClient}.tsx
- Migration: none
- Env added: none
- Review notes / open questions: The page builder now uses a reusable accessible modal that searches names/paths, filters images versus other existing objects, previews tenant media, and returns the complete `{objectKey,url}` asset contract to callers. Hero and feature-band controls store the public URL because the current public renderers already accept absolute URLs; future private-bucket consumers can select `objectKey` from the same callback. Upload-new uses the existing SCRUM-164 `tenant-media` byte/MIME policy and the main asset-library copy and dropzone now advertise that exact image-only 10 MB policy rather than the previous unsupported video/50 MB promise. Listing still shows legacy non-image objects so administrators can discover them, but new uploads fail closed through `validateUploadWithBytes`. Claude should confirm whether video support is desired later; it requires an explicit additive shared policy change and safe video byte signatures, not a picker-only exception.

## SCRUM-105 Default-off subdomain tenant routing
- Status: stubbed(flag=SUBDOMAIN_TENANT_ROUTING_ENABLED)
- Files: apps/admin/{middleware.ts,.env.example,lib/{subdomainRouting*,tenant.ts}}, vite.config.ts, .env.example, CLAUDE.md
- Migration: none
- Env added: SUBDOMAIN_TENANT_ROUTING_ENABLED=false, LUME_ROOT_DOMAIN=..., VITE_SUBDOMAIN_TENANT_ROUTING_ENABLED=false
- Review notes / open questions: The existing Next middleware now injects `x-lume-tenant` only when the server flag is exactly true and Host is exactly one valid, non-reserved tenant label below `LUME_ROOT_DOMAIN`; host-derived scope overrides a caller header on those tenant hosts. With the flag absent/false, execution proceeds directly to the unchanged session refresh and `/admin` redirect gate. The Vite API proxy preserves the browser Host only behind its separate default-off development flag, and CLAUDE.md documents `default.lume.local`. Direct public APIs retain their pre-existing permissive host fallback because chat/vehicles are intentionally excluded from middleware and removing it would be a behavior regression. This remains stubbed until wildcard DNS/TLS and Vercel project-domain provisioning exist. Claude must first regression-test unauthenticated and authenticated `/admin/*` traffic with the flag both off and on, then decide whether to migrate the deprecated Next 16 `middleware.ts` to `proxy.ts` as a separate security-focused change; this ticket deliberately avoids coupling an auth-boundary migration to tenancy rollout.
