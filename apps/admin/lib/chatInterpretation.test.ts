...
describe("interpretation schema: what it accepts", () => {
  ...
  it("accepts a fenced reply, because models fence JSON", () => {
    expect(parseChatInterpretation("```json\n" + plan() + "\n```")).not.toBeNull();
  });
  ...
  it("keeps clauses it cannot express instead of dropping them", () => {
    ...
    expect(parsed?.unsupportedClauses).toEqual(["book a test drive for Saturday"]);
  });

  it("accepts an ordinal reference within range", () => {
    const parsed = parseChatInterpretation(
      plan({ kind: "reference", reference: { kind: "ordinal", position: 3 } }),
    );
  ...
  it("accepts a comparison of two positions", () => {
    const parsed = parseChatInterpretation(
      plan({ kind: "reference", reference: { kind: "compare", positions: [1, 2] } }),
    );
...
describe("interpretation schema: what it refuses", () => {
  ...
  it("refuses an unknown intent kind", () => {
    expect(parseChatInterpretation(plan({ kind: "delete_everything" }))).toBeNull();
  });
  ...
  it("refuses an unknown filter field rather than skipping it", () => {
    ...
    expect(
      parseChatInterpretation(plan({ setFilters: { make: "BMW", vin: "XYZ" } })),
    ).toBeNull();
  });
  ...
  it("refuses a filter with the wrong type", () => {
    ...
    expect(parseChatInterpretation(plan({ setFilters: { make: 7 } }))).toBeNull();
  });
  ...
  it("refuses a comparison of fewer than two positions", () => {
    expect(
      parseChatInterpretation(
        plan({ kind: "reference", reference: { kind: "compare", positions: [1] } }),
      ),
    ).toBeNull();
  });
  ...
  it("refuses an unknown clarify reason", () => {
    expect(
      parseChatInterpretation(plan({ kind: "clarify", clarifyReason: "because" })),
    ).toBeNull();
  });
  ...
  it("has no field capable of carrying an action, id or destination", () => {
    // The structural guarantee: even a fully compliant plan cannot ask for a
    // navigation, a vehicle, a URL or a capability.
    const parsed = parseChatInterpretation(
      plan({
        action: "navigate",
    ...
    expect(serialized).not.toContain("navigate");
    expect(serialized).not.toContain("11111111");
    expect(serialized).not.toContain("/admin");
    expect(serialized).not.toContain("vehicle.delete");
  });
});
...
describe("interpreter prompt and context stay bounded", () => {
  ...
  it("says 'none' rather than inventing a search that is not active", () => {
    ...
    expect(buildInterpreterContextPrompt(context)).toContain("Active search: none");
  });
});
...
describe("shadow comparison records fields, never content", () => {
  ...
  it("names which fields differ without recording their values", () => {
    const comparison = compareShadowInterpretation({
      deterministic: { kind: "search", filters: { make: "BMW" }, hasReference: false },
      candidate,
    });
  ...
  it("reports agreement when both readings match", () => {
    const agreeing = parseChatInterpretation(plan({ setFilters: { make: "BMW" } }))!;
    const comparison = compareShadowInterpretation({
      deterministic: { kind: "search", filters: { make: "BMW" }, hasReference: false },
      candidate: agreeing,
    });
  ...
  it("notices a reference disagreement", () => {
    const reference = parseChatInterpretation(
      plan({ kind: "reference", reference: { kind: "ordinal", position: 2 } }),
    )!;
    const comparison = compareShadowInterpretation({
...
describe("shadow mode is off unless two separate gates are opened", () => {
  it("is off by default", () => {
    expect(isShadowInterpretationEnabled("demo", undefined, undefined)).toBe(false);
  });
  ...
  it("stays off with the flag alone", () => {
    ...
    expect(isShadowInterpretationEnabled("demo", "true", undefined)).toBe(false);
  });

  it("stays off for a tenant outside the allowlist", () => {
    expect(isShadowInterpretationEnabled("default", "true", "demo")).toBe(false);
  });

  it("is on only for an allowlisted tenant in an enabled environment", () => {
    expect(isShadowInterpretationEnabled("demo", "true", "demo,other")).toBe(true);
...
describe("gold set is usable as evidence", () => {
  ...
  it("reports denominators so no percentage can be quoted bare", () => {
    ...
    expect(denominators.byPartition["held-out"].conversations).toBeGreaterThan(0);
  });
  ...
  it("covers the historical failure classes", () => {
    ...
    for (const failure of ["cap must not survive", "numeral ordinal", "cadillac camry", "half"]) {
