...
export type ExecutedVehicleStatusCommand =
  | { ok: true; alreadyExecuted: boolean; vehicleId: string; label: string; previousStatus: string; nextStatus: string }
  | { ok: false; reason: "not_found" | "expired" | "stale" | "failed" | "unavailable"; error: string };
...
export function parseVehicleStatusCommandReceipt(value: unknown): ExecutedVehicleStatusCommand {
  if (typeof value !== "object" || value === null || Array.isArray(value)) {
    return { ok: false, reason: "unavailable", error: "Unable to execute the reviewed command." };
  }
  const record = value as Record<string, unknown>;
  ...
  if (status === "executed") {
    const vehicleId = typeof record.vehicleId === "string" ? record.vehicleId : "";
    const nextStatus = typeof record.nextStatus === "string" ? record.nextStatus : "";
    const previousStatus = typeof record.previousStatus === "string" ? record.previousStatus : "";
    if (!vehicleId || !nextStatus || !previousStatus) {
      return { ok: false, reason: "unavailable", error: "Unable to execute the reviewed command." };
    }
    return {
      ok: true,
