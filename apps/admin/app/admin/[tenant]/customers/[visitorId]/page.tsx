...
export default async function CustomerProfilePage({ params }: Props) {
  ...
  return <div className="space-y-6">
    ...
    <section className="rounded-xl border p-4"><h2 className="font-semibold">Saved vehicles</h2>{!saves.data?.length ? <p className="mt-3 text-sm text-muted-foreground">No saved vehicles.</p> : <ul className="mt-3 space-y-2 text-sm">{saves.data.map((save) => <li key={save.vehicle_id} className="flex justify-between"><span>{save.vehicle_id}</span><time>{format(save.created_at)}</time></li>)}</ul>}</section>
