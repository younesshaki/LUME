    ...
    case "star": {
      ...
      const star = (outerRadius: number) => {
        ...
        for (let index = 0; index < 5; index += 1) {
          ...
          vertices.push(
            `${cx + outerRadius * Math.cos(outerAngle)}px ${cy + outerRadius * Math.sin(outerAngle)}px`
          );
          vertices.push(
            `${cx + outerRadius * 0.42 * Math.cos(innerAngle)}px ${cy + outerRadius * 0.42 * Math.sin(innerAngle)}px`
          );
        }
