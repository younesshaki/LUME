...
import "@/experience/loaders/preloader/styles.css";
import { useCdnImage } from "@/config/cdn";
import { preloadGateSoundActions } from "./PreloadGate.sounds";
...
export default function PreloadGate({ onStart }: PreloadGateProps) {
  ...
  const handleAuthSubmit = async (e: React.FormEvent) => {
    ...
    play(preloadGateSoundActions.submit);
    setSubmitting(true);
  ...
  const handleStart = () => {
    ...
    setIsStarting(true);
    play(preloadGateSoundActions.start);
        ...
        <div ref={authRef} className="pgAuth">
          <form className="pgAuth__form" onSubmit={handleAuthSubmit} noValidate>
            ...
            <button
              ...
              onMouseEnter={!busy ? () => play(preloadGateSoundActions.hover) : undefined}
            >
              {submitting ? "..." : "Enter"}
            </button>
          </form>
      ...
      {phase === "ready" ? (
        ...
        >
          ...
          <button
            ...
            type="button"
            onMouseEnter={isStarting ? undefined : () => play(preloadGateSoundActions.hover)}
            onFocus={isStarting ? undefined : () => play(preloadGateSoundActions.hover)}
            onClick={handleStart}
            disabled={isStarting}
          >
            Play
