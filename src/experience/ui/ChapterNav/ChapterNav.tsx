...
import type { Part } from "@/experience/parts";
import { chapterNavSoundActions } from "./ChapterNav.sounds";
...
function FancyButton({
  ...
  const handleClick = () => {
    ...
    play(chapterNavSoundActions.click);
    onClick();
  };
  ...
  return (
    <button
      type="button"
      className="button"
      onMouseEnter={disabled ? undefined : () => play(chapterNavSoundActions.hover)}
      onFocus={disabled ? undefined : () => play(chapterNavSoundActions.hover)}
      onClick={handleClick}
      disabled={disabled}
    >
      <span className="fold" />
...
function ChapterSelect({
  ...
  const handleToggle = () => {
    if (disabled) return;
    play(chapterNavSoundActions.selectOpen);
    setOpen((prev) => !prev);
  };
            ...
            {options.map((option) => {
              ...
              return (
                <button
                  ...
                  onMouseEnter={() => play(chapterNavSoundActions.hover)}
                  onFocus={() => play(chapterNavSoundActions.hover)}
                  onClick={() => {
                    play(chapterNavSoundActions.selectChoose);
